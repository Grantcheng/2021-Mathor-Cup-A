...
class BHTARIMA(TransformerMixin):
    ...
    def __init__(self, p, d, q, taus, Rs, K, tol, seed=None, Us_mode=4,
    ...
    def fit(self, x, *args, **kwargs):
        ...
        self._N = len(x.shape) - 1
        self.T = x.shape[-1]
        assert x.shape[0] + 1 - self._taus[1] - self._p - self._q - self._d > 0, \
            'Require: len(ts) + 1 - tau - p - q - d > 0'
        # check Rs parameters
        M = 0
