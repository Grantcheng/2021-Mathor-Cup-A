...
optimizer = BayesSearchCV(
    SGDRegressor(penalty='elasticnet', random_state=502383, epsilon=0.01, learning_rate='adaptive',
                 early_stopping=True),
    {
        'alpha': Real(1e-6, 1, prior='log-uniform'),
        'l1_ratio': Real(0, 1, prior='uniform'),
        'eta0': Real(0.0001, 0.5, prior='log-uniform'),
    },
    n_iter=32, cv=5
)
...
with open('raw/Q1_mdl_bayes_sgd_regressor.pkl', 'wb') as f:
    pickle.dump(optimizer, f)

# %% Import trained model.
with open('raw/Q1_mdl_bayes_sgd_regressor.pkl', 'rb') as f:
...
fig.savefig('results/Q1_err_bayes_sgd_regressor.svg')
plt.close(fig)
